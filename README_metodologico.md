...
**Metriche drift (due formulazioni).** (1) Frobenius normalizzata per dimensione: `‖H_ckpt − H_base‖_F / (N × d)` — magnitudine media per elemento. (2) Frobenius relativa: `‖H_ckpt − H_base‖_F / ‖H_base‖_F` — scala-invariante. Entrambe calcolate separatamente su stimoli matematici e di controllo. La Frobenius relativa è la metrica di confronto con T16 (nf4_degradation.py): essendo scala-invariante, il confronto è valido nonostante T16 usi hook HF nativi (senza fold_ln) mentre RQ3 usi TransformerLens (fold_ln=True). TransformerLens non può wrappare modelli NF4 (le operazioni di fold richiedono accesso diretto ai pesi, incompatibile con tensori quantizzati BitsAndBytes). Le metriche non decompongono la trasformazione in componenti rotazionali, scalari e traslazionali — solo la distanza totale è misurata.
